...
use filecoin_proofs::{PrivateReplicaInfo, PublicReplicaInfo, generate_window_post, verify_window_post, get_seal_inputs};
...
pub const CC_PREFIX: &str = "LOTUS_CC_SECTOR_";
...
lazy_static! {
    ...
    pub static ref POREP_CONFIG: PoRepConfig = get_porep_config();
    pub static ref WINNING_POST_CONFIG: PoStConfig=get_winning_post_config();
    pub static ref WINDOW_POST_CONFIG: PoStConfig=get_window_config();
...
fn get_piece_info() ->PieceInfo {
    ...
    let _ =  comm_p.write_bytes(&mut commitment);
...
fn set_last_sector_id(sector_id: &u64) {
    ...
    let _ = file.write(new_value.as_bytes());
}
...
fn cache_porep_params<Tree: 'static + MerkleTreeTrait>(porep_config: PoRepConfig) {
    ...
    let public_params = public_params (
        // &porep_config
        PaddedBytesAmount::from(porep_config.clone()),
        usize::from(PoRepProofPartitions::from(porep_config.clone())),
        porep_config.clone().porep_id,
        FIXED_API_VERSION,
    )
        .unwrap();
...
fn get_porep_config() -> PoRepConfig {
...
fn get_winning_post_config() -> PoStConfig {
    let sector_size = get_data_size();
...
fn get_window_config() -> PoStConfig {
    let sector_size = get_data_size();
...
fn created_cached_dt_tree<Tree: 'static + MerkleTreeTrait>() -> BinaryMerkleTree<DefaultPieceHasher> {
    ...
    let sp = compound_proof::SetupParams {
        vanilla_params: setup_params(
            // &POREP_CONFIG
            PaddedBytesAmount::from(POREP_CONFIG.sector_size),
            usize::from(PoRepProofPartitions::from(POREP_CONFIG.partitions)),
            (*POREP_CONFIG).porep_id,
            FIXED_API_VERSION,
        ).unwrap(),
        partitions: Some(usize::from(PoRepProofPartitions::from(POREP_CONFIG.partitions))),
        priority: false,
    };
    ...
    let _ = create_dir_all(cache_path_str.clone());
...
fn copy_cached_tree_d(sector_id: u64) {
    ...
    let _ = fs::symlink(src_path, dst_path);
...
{
    let sector_bytes = PaddedBytesAmount::from(POREP_CONFIG.sector_size);
...
pub fn precommit_phase1<Tree: 'static + MerkleTreeTrait>()
{
    ...
    let _ = copy_cached_tree_d(sector_id);
    ...
    let _ = &(*SECTORS_FOR_HASH).write().unwrap().push_back(sector_id);
    let _ = &(*METAS_FOR_HASH).write().unwrap().insert(sector_id, meta);
...
pub fn precommit_phase2<Tree: 'static + MerkleTreeTrait>() {
    ...
    if &(*SECTORS_FOR_HASH).read().unwrap().len() > &0 {
        ...
        let _ = &(*SECTORS_FOR_PROVE).write().unwrap().push_back(sector_id);
        let _ = &(*METAS_FOR_PROVE).write().unwrap().insert(sector_id, meta);

        println!("Finish sector {:?} precommit2 @{:?}", sector_id, Instant::now());

        if *TEST_WINNING{
            let _ = &(*REPLICA_INFO_FIELDS_MAP).write().unwrap().insert(sector_id,
                                                               replica_info_fields {
    ...
    if &(*SECTORS_FOR_PROVE).read().unwrap().len() > &0 {
        ...
        if do_bellman {
            ...
            if phase1_output.is_ok() {
                ...
                let aggregate_version = groth16::aggregate::AggregateVersion::V2;
                let aggregate_proof =
                    aggregate_seal_commit_proofs::<Tree>(&*POREP_CONFIG, &seeds, &seeds, commit_outputs.as_slice(), aggregate_version).unwrap();
                let res = verify_aggregate_seal_commit_proofs::<Tree>(&*POREP_CONFIG, aggregate_proof, &seeds, &seeds, commit_inputs, aggregate_version).unwrap();
...
fn main() {
    ...
    if !restart {
        let _ = remove_dir_all(staging_folder.clone());
        let _ = create_dir_all(staging_folder.clone());
        let _ = create_staging_file();
    }
    ...
    if !restart {
        let _ = remove_dir_all(sealed_folder.clone());
        let _ = create_dir_all(sealed_folder.clone());
    }else {
    ...
    if !restart {
        ...
        let _ = create_dir_all(cache_folder.clone());
    }else {
